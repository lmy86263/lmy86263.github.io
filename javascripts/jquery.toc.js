...
(function($) {
    ...
    // ignore tocNumber, stop generating order for title automaticly
    ...
    $.fn.toc = function(settings) {
        ...
        tocContainer.find(firstHead).each(function() {
            ...
            h1.nextUntil(firstHead).filter(secondHead).each(function() {
                ...
                var h2_levelHTML = '';
                var h2_innerSection = 0;
                var h2 = $(this);
                h2.nextUntil(secondHead).filter(thirdHead).each(function() {
                    ++h2_innerSection;
                    var h2_anchorId = anchorId + "-" + h2_innerSection;
                    $(this).attr('id', h2_anchorId);
                    h2_levelHTML += createLevelHTML(h2_anchorId,
                        tocLevel + 2,
                        tocSection + h2_innerSection,
                        // itemNumber + '.' + innerSection,
                        $(this).text());
                });
                if (h2_levelHTML) {
                    h2_levelHTML = '<ul>' + h2_levelHTML + '</ul>\n';
                }
                levelHTML += h2_levelHTML
